...
def make_task_execution_result(
    task_id: str,
    task: str,
    status: str,
    input_data: dict,
    ...
    if not isinstance(input_data, dict):
        raise ValueError("task input_data must be a JSON object")
    return {
        "task_id": task_id,
        "task": task,
        "status": status,
...
def make_error_payload(exc: Exception) -> dict:
    # Stable error codes let B2/B3 handle failures without depending on exception text.
    message = str(exc)
    if isinstance(exc, FileNotFoundError):
        ...
        if "format" in lowered or "suffix" in lowered or "supports" in lowered:
            code = "UNSUPPORTED_FORMAT"
        elif "escapes data root" in lowered:
            code = "PATH_ESCAPE"
        else:
            code = "INVALID_ARGUMENT"
    else:
        code = "EXECUTION_ERROR"
    return {"type": type(exc).__name__, "code": code, "message": message}


def ensure_json_object(value: Any, label: str = "value") -> None:
    # Skill outputs are persisted and embedded in ToolMessage.content as JSON.
