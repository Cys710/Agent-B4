...
| B2 | `code/b2_run_skill.py` | 独立运行五个基础 Skill，并提供五个扩展 Skill。 |
...
失败时 `status` 为 `error`、`output` 为 `null`，`error` 包含异常类型、错误码和错误信息。当前通用错误码包括 `INVALID_ARGUMENT`、`UNSUPPORTED_FORMAT`、`PATH_ESCAPE`、`FILE_NOT_FOUND`、`PERMISSION_DENIED`、`TIMEOUT` 和 `EXECUTION_ERROR`。
...
B2 的基础验收工具集包含 `calculator`、`file_reader`、`local_file_search`、`table_analyzer`、`format_converter` 五个 Skill。扩展部分在不改变基础命令入口的前提下，增加 `document_reader`、`read_then_convert`、`python_sandbox`、`python_unit_test_sandbox` 和 `web_search` 五个 Skill，并同步注册到 B3 的 `extended_tools` 工具集，便于后续被 Agent 以 tool call 方式调用。

### 3.1 通用命令行输入

| 参数 | 说明 |
|---|---|
| `--skill` | Skill 名称：`calculator`、`file_reader`、`local_file_search`、`table_analyzer`、`format_converter`、`document_reader`、`read_then_convert`、`python_sandbox`、`python_unit_test_sandbox` 或 `web_search`。 |
...
| document_reader | `data/tool_inputs/tool_input_document_reader.json` | `path`：PDF/DOCX/PPTX 路径；`max_chars`：最大返回字符数。 | `tool_input_document_reader_error.json` |
| read_then_convert | `data/tool_inputs/tool_input_read_then_convert.json` | `path`、`target_format`、`max_chars`、`output_filename`。 | `tool_input_read_then_convert_error.json` |
| python_sandbox | `data/tool_inputs/tool_input_python_sandbox.json` | `code`、`timeout_seconds`、`max_output_chars`。 | `tool_input_python_sandbox_error.json` |
| python_unit_test_sandbox | `data/tool_inputs/tool_input_python_unit_test_sandbox.json` | `code`、`tests`、`timeout_seconds`、`max_output_chars`；每个测试包含 `expression` 和 `expected`。 | `tool_input_python_unit_test_sandbox_error.json` |
| web_search | `data/tool_inputs/tool_input_web_search.json` | `query`、`top_k`、`timeout_seconds`；测试样例可传 `mock_results` 固定联网结果。 | `tool_input_web_search_error.json` |
...
```bash
python b2_run_skill.py --skill document_reader --input ../data/tool_inputs/tool_input_document_reader.json --outdir ../outputs/B2_skills
python b2_run_skill.py --skill read_then_convert --input ../data/tool_inputs/tool_input_read_then_convert.json --outdir ../outputs/B2_skills
python b2_run_skill.py --skill python_sandbox --input ../data/tool_inputs/tool_input_python_sandbox.json --outdir ../outputs/B2_skills
python b2_run_skill.py --skill python_unit_test_sandbox --input ../data/tool_inputs/tool_input_python_unit_test_sandbox.json --outdir ../outputs/B2_skills
python b2_run_skill.py --skill web_search --input ../data/tool_inputs/tool_input_web_search.json --outdir ../outputs/B2_skills
```

### 3.4 B2 扩展说明

`local_file_search` 在基础文件检索能力上做了增强：会从查询语句中提取规范化关键词，按词频、路径命中和片段命中综合排序，并在结果中返回 `query_terms`、`line_number`、`matched_terms`、`score` 和 `snippet`。这样 B3/B1 调用搜索工具后，不只知道哪个文件命中，还能定位命中行和命中词，便于模型继续读取或引用局部内容。

`document_reader` 用于读取本地 PDF、DOCX 和 PPTX 文档文本。它保持和 `file_reader` 相同的路径安全边界，只允许读取配置的 `data_root` 内文件；DOCX/PPTX 通过解析 Office Open XML 中的文本节点实现，PDF 通过解析常见文本流实现，并在结果中返回 `file_type`、`metadata`、`warnings` 和 `truncated`，便于 Agent 判断是否需要继续分块或换用 OCR。

`read_then_convert` 是组合型 Skill，内部先调用 `file_reader` 读取本地 txt/md 文件，再调用 `format_converter` 把文本转换成 Markdown 或 JSON，并把转换结果写入输出目录。它用于验证 B2 工具函数可以被复用和编排，不需要在 Agent 层重复实现“读取后转换”的业务流程。

`python_sandbox` 用于执行短 Python 片段。它先通过 AST 检查拒绝 `import`、`open`、`eval`、`exec`、双下划线属性等高风险语法或名称，再用独立 Python 进程运行代码，并限制 `timeout_seconds`、`max_output_chars` 和临时工作目录。该工具只面向课程演示中的轻量计算、列表处理和文本处理，不作为通用安全沙箱。

`python_unit_test_sandbox` 用于在同一安全边界下验证用户代码。它接收待测 `code` 和若干 `{expression, expected}` 测试用例，先复用 `python_sandbox` 的 AST 禁止规则，再在独立 Python 进程中执行代码并逐个求值测试表达式。工具运行成功不代表业务全部正确，具体正确性由 `passed_count`、`failed_count`、`all_passed` 和 `cases` 说明，因此适合展示“Agent 可以运行代码并定位失败用例”的能力。

`web_search` 用于给 Agent 增加联网检索能力。工具接收 `query`、`top_k` 和 `timeout_seconds`，真实运行时只访问固定的 DuckDuckGo HTML 搜索端点，并把联网请求放入独立 Python 子进程中执行；主进程只接收受限大小的 HTML 文本并解析标题、链接和摘要。课堂测试或离线演示时可传入 `mock_results`，使 B2、B3、B1 端到端流程得到稳定、可复现的搜索结果。这样既能体现“联网工具函数”的接口设计，也能避免展示现场因为 DNS、校园网或搜索站点限流导致流程失败。

B2 入口会校验输入 JSON 顶层必须是对象，成功输出也必须是可序列化 JSON 对象。异常不会导致 CLI 崩溃，而是统一写入 SkillResult 的 `error` 字段，包含 `type`、`code`、`message`，便于 B3 继续封装为 ToolMessage。

扩展 Skill 已加入 `configs/tools.yaml` 的 `extended_tools`。B3 对接验证命令如下：

```bash
python b3_tool_layer.py --tools_config ../configs/tools.yaml --toolset extended_tools --export_schema --outdir ../outputs/B3_tools/extended
python b3_tool_layer.py --tools_config ../configs/tools.yaml --toolset extended_tools --tool_calls ../data/messages/b3_tool_call_document_reader_valid.json --execute --outdir ../outputs/B3_tools/document_reader
python b3_tool_layer.py --tools_config ../configs/tools.yaml --toolset extended_tools --tool_calls ../data/messages/b3_tool_call_read_then_convert_valid.json --execute --outdir ../outputs/B3_tools/read_then_convert
python b3_tool_layer.py --tools_config ../configs/tools.yaml --toolset extended_tools --tool_calls ../data/messages/b3_tool_call_python_sandbox_valid.json --execute --outdir ../outputs/B3_tools/python_sandbox
python b3_tool_layer.py --tools_config ../configs/tools.yaml --toolset extended_tools --tool_calls ../data/messages/b3_tool_call_python_unit_test_sandbox_valid.json --execute --outdir ../outputs/B3_tools/python_unit_test_sandbox
python b3_tool_layer.py --tools_config ../configs/tools.yaml --toolset extended_tools --tool_calls ../data/messages/b3_tool_call_web_search_valid.json --execute --outdir ../outputs/B3_tools/web_search
```

基础工具集导出时预期 `tool_count=5`；扩展工具集导出时预期 `tool_count=10`。
...
| `outputs/B2_skills/local_file_search_result.json` | JSON 对象（SkillResult） | 文件搜索结果；包含查询词、路径、匹配分数、命中行、命中词和片段。 |
...
| `outputs/B2_skills/document_reader_result.json` | JSON 对象（SkillResult） | PDF/DOCX/PPTX 文档读取结果；包含正文、来源、文件类型、元数据、告警和截断标志。 |
| `outputs/B2_skills/read_then_convert_result.json` | JSON 对象（SkillResult） | 读取文件后的转换结果，包含来源、截断标志、目标格式和生成文件路径。 |
| `outputs/B2_skills/python_sandbox_result.json` | JSON 对象（SkillResult） | 受限 Python 片段运行结果，包含 stdout、stderr、returncode、超时和截断状态。 |
| `outputs/B2_skills/python_unit_test_sandbox_result.json` | JSON 对象（SkillResult） | 受限 Python 单元测试结果，包含通过数、失败数、逐用例实际值和错误信息。 |
| `outputs/B2_skills/web_search_result.json` | JSON 对象（SkillResult） | 联网搜索结果；包含查询语句、结果来源、排序后的标题、链接和摘要。 |
...
| `data/messages/b3_tool_call_recoverable_errors.json` | B3 扩展二样例，包含参数名前后空格、数字字符串、布尔字符串、数组 JSON 字符串、args JSON 字符串等可恢复错误。 |
| `data/messages/b3_tool_call_unrecoverable_errors.json` | B3 扩展二样例，包含缺少必填参数和未知工具等不可恢复错误。 |
| `data/messages/b3_tool_call_cache_repeated.json` | B3 扩展三样例，包含 calculator、file_reader、table_analyzer 的重复 tool call，用于验证同批次缓存命中和参数顺序无关的缓存 key。 |
| `data/messages/b3_tool_call_stats_batch.json` | B3 扩展四样例，包含 10 条混合成功/失败 tool call，用于验证调用次数、失败率、平均耗时和按工具聚合统计。 |
| `data/messages/b3_schema_accuracy_batch.json` | B3 扩展五样例，包含 5 个用户任务、3 种 schema 描述版本和对应模型 tool_call 输出，用于比较工具调用准确率。 |
...
B3 扩展一：自动从 Python 函数签名生成或合并 tools_schema。`--schema_source config` 为默认值，完全使用 `tools.yaml`；`inferred` 使用 Skill 函数签名和类型注解推断参数；`merged` 使用函数签名确定参数和 required，同时保留 `tools.yaml` 中的人工描述。
```bash
python b3_tool_layer.py --tools_config ../configs/tools.yaml --toolset basic_tools --export_schema --schema_source inferred --outdir ../outputs/B3_schema_inferred_fix3_1
python b3_tool_layer.py --tools_config ../configs/tools.yaml --toolset basic_tools --export_schema --schema_source merged --outdir ../outputs/B3_schema_merged_fix3_1
```

自动 schema 导出会额外生成 `tool_schema_audit.json`，用于检查 `tools.yaml` 与真实 Python 函数签名是否一致，包括缺失参数、多余参数、类型不一致、required 不一致和由 B3 自动注入的 `data_root` / `output_dir`。
...
B3 扩展二：可恢复错误有限重试。默认开启恢复，会在执行前按 schema 修复轻微格式错误；`--disable_recovery` 可关闭该功能做对照实验。
```bash
python b3_tool_layer.py --tools_config ../configs/tools.yaml --toolset basic_tools --tool_calls ../data/messages/b3_tool_call_recoverable_errors.json --execute --outdir ../outputs/B3_recoverable_json_args_fix3_2
python b3_tool_layer.py --tools_config ../configs/tools.yaml --toolset basic_tools --tool_calls ../data/messages/b3_tool_call_unrecoverable_errors.json --execute --outdir ../outputs/B3_unrecoverable_fix3_2
python b3_tool_layer.py --tools_config ../configs/tools.yaml --toolset basic_tools --tool_calls ../data/messages/b3_tool_call_recoverable_errors.json --execute --disable_recovery --outdir ../outputs/B3_recovery_disabled_fix3_2
```

可恢复范围包括：`args` 为 JSON 字符串、参数名前后空格、工具名前后空格、字符串数字转 integer/number、字符串 true/false 转 boolean、数组或对象 JSON 字符串解析、路径等字符串参数去除首尾空格。缺少必填参数和未知工具不会被猜测修复。错误样例不会让 CLI 崩溃；错误会写入 `tool_messages.json` 和 `tool_call_log.jsonl` 中的 `status=error` SkillResult。

B3 扩展三：tool_call 结果缓存。默认开启缓存，只缓存 `status=success` 的 SkillResult；缓存 key 由工具名和规范化后的参数 JSON 生成，因此参数顺序不同但语义相同的调用会命中同一个缓存。`--disable_cache` 可关闭缓存做对照实验；`--cache_dir` 可指定跨运行共享的缓存目录。
```bash
python b3_tool_layer.py --tools_config ../configs/tools.yaml --toolset basic_tools --tool_calls ../data/messages/b3_tool_call_cache_repeated.json --execute --outdir ../outputs/B3_cache_repeated_fix3_3
python b3_tool_layer.py --tools_config ../configs/tools.yaml --toolset basic_tools --tool_calls ../data/messages/b3_tool_call_cache_repeated.json --execute --disable_cache --outdir ../outputs/B3_cache_disabled_fix3_3
python b3_tool_layer.py --tools_config ../configs/tools.yaml --toolset basic_tools --tool_calls ../data/messages/b3_tool_call_cache_repeated.json --execute --cache_dir ../outputs/B3_shared_cache_fix3_3 --outdir ../outputs/B3_cache_shared_first_fix3_3
python b3_tool_layer.py --tools_config ../configs/tools.yaml --toolset basic_tools --tool_calls ../data/messages/b3_tool_call_cache_repeated.json --execute --cache_dir ../outputs/B3_shared_cache_fix3_3 --outdir ../outputs/B3_cache_shared_second_fix3_3
```

缓存执行记录写入 `tool_call_log.jsonl` 的 `cache` 字段，包括 `enabled`、`hit`、`cache_key` 和 `cache_path`。默认缓存目录是当前 `outdir/tool_cache/`；指定 `--cache_dir` 后可在多次运行之间复用缓存。

B3 扩展四：工具调用统计。执行 tool calls 时会自动生成 `tool_stats_report.json`，统计整体和每个工具的调用次数、成功数、失败数、失败率、平均/最小/最大耗时，并附带缓存命中和恢复成功次数。也可以用 `--stats_report --stats_from` 从已有 `tool_call_log.jsonl` 重新生成统计报告。
```bash
python b3_tool_layer.py --tools_config ../configs/tools.yaml --toolset basic_tools --tool_calls ../data/messages/b3_tool_call_stats_batch.json --execute --outdir ../outputs/B3_stats_batch_fix3_4
python b3_tool_layer.py --tools_config ../configs/tools.yaml --toolset basic_tools --stats_report --stats_from ../outputs/B3_stats_batch_fix3_4/tool_call_log.jsonl --outdir ../outputs/B3_stats_from_log_fix3_4
```

统计样例 `b3_tool_call_stats_batch.json` 包含 10 条调用：5 条成功、5 条失败。预期总体统计为 `total_calls=10`、`success_count=5`、`error_count=5`、`failure_rate=0.5`；按工具统计中 calculator、file_reader、local_file_search、table_analyzer 各 2 次调用且失败率 0.5，format_converter 成功率 100%，未知工具 web_search 失败率 1.0。

B3 扩展五：比较不同 `tools_schema` 描述对模型工具调用准确率的影响。`--schema_experiment` 会基于同一份工具 schema 生成 `clear`、`concise`、`ambiguous` 三种描述版本，并使用批量样例中的模型 tool_call 输出计算工具名准确率、参数准确率、完整调用准确率和解析错误率。样例中的预测是 fixture，可将真实 B4/模型生成的 AIMessage tool_calls 填入同一 JSON 结构后复用评测。
```bash
python b3_tool_layer.py --tools_config ../configs/tools.yaml --toolset basic_tools --schema_experiment --experiment_batch ../data/messages/b3_schema_accuracy_batch.json --outdir ../outputs/B3_schema_accuracy_fix3_5
```

扩展五样例预期结果：`clear` 的 `exact_call_accuracy=1.0`，`concise=0.6`，`ambiguous=0.2`；报告会标记最佳 schema 版本，并输出每个 case 的 expected/predicted 对照。
...
| `outputs/B3_tools/tool_schema_report.json` | JSON 对象 | schema 导出报告，包含 toolset、工具数量、schema_source 和 schema_audit_summary。 |
| `outputs/B3_tools/tool_schema_audit.json` | JSON 对象 | B3 扩展一输出，记录每个工具的配置参数、函数签名推断参数、差异项和自动注入参数。 |
| `outputs/B3_tools/tool_messages.json` | JSON 数组 | 本次执行生成的 ToolMessage 列表。 |
| `outputs/B3_tools/tool_call_log.jsonl` | JSONL | 每个 tool call 的完整执行记录，含未转义 SkillResult、状态、参数、耗时、B3 扩展二的 `recovery` 修复记录和 B3 扩展三的 `cache` 命中记录。 |
| `outputs/B3_tools/tool_cache/*.json` | JSON 对象 | 默认缓存目录，保存成功 tool call 的缓存 key、工具名、规范化参数、SkillResult 和创建时间；使用 `--cache_dir` 时写入指定目录。 |
| `outputs/B3_tools/tool_stats_report.json` | JSON 对象 | B3 扩展四输出，包含 overall 总体统计和 by_tool 按工具统计：调用次数、成功/失败数、失败率、平均/最小/最大耗时、缓存命中率和恢复成功次数。 |
| `outputs/B3_tools/schema_accuracy_report.json` | JSON 对象 | B3 扩展五输出，包含不同 schema 描述版本的 tool_name_accuracy、required_args_accuracy、exact_call_accuracy、parse_error_rate 和逐 case 对比。 |
| `outputs/B3_tools/schema_variants/*.json` | JSON 数组 | B3 扩展五输出，保存 clear、concise、ambiguous 等 schema 描述版本，便于和模型实际输出对应归档。 |
...
| `data/runtime_input_b2_document_reader.json` | B2 document_reader 端到端任务，读取 PDF 文档并总结要点。 |
| `data/runtime_input_b2_python_unit_test_sandbox.json` | B2 python_unit_test_sandbox 端到端任务，验证用户代码并总结通过/失败测试用例。 |
| `data/runtime_input_b2_web_search.json` | B2 web_search 端到端任务，使用 mock 搜索结果验证 Agent 调用联网搜索 Skill 的链路。 |
| `data/runtime_input_b5_auto_memory.json` | B5 自动检索 memory 集成任务，使用 `memory_retrieval_mode=vector` 自动召回相关记忆。 |
| `data/runtime_input_b5_keyword_memory.json` | B5 自动检索 memory 集成任务，使用 `memory_retrieval_mode=keyword` 回归关键词召回链路。 |
...
```bash
python b1_agent_runtime.py --input ../data/runtime_input_b5_auto_memory.json --tools_config ../configs/tools.yaml --memory_config ../configs/memory.yaml --model_config ../configs/model.yaml --llm_mode mock --outdir ../outputs/B1_b5_auto_memory
python b1_agent_runtime.py --input ../data/runtime_input_b5_keyword_memory.json --tools_config ../configs/tools.yaml --memory_config ../configs/memory.yaml --model_config ../configs/model.yaml --llm_mode mock --outdir ../outputs/B1_b5_keyword_memory
```

integrated 输入可选字段：`memory_retrieval_mode` 默认为 `manual`，也可设为 `keyword` 或 `vector`；`memory_top_k` 控制自动召回数量；`use_global_memory` 在自动检索模式下表示是否把 global memory 纳入候选集合。自动检索命中后，B1 会把召回的 memory_id 交给 B5 `load_memory` 注入 SystemMessage，并在 `selected_memory.json` 和 `trace.json.memory_retrieval` 中记录结果。
...
| `outputs/B1_runtime/selected_memory.json` | JSON 对象 | B1 调用 B5 后保存的 memory 选择结果；自动检索模式会包含 `retrieval_mode`、`top_k` 和 `retrieval_results`。 |
...
| `document_reader_result.json` | `b2_run_skill.py` | document_reader 最近一次 SkillResult，包含文档正文、文件类型、元数据和告警。 | JSON 对象 |
| `read_then_convert_result.json` | `b2_run_skill.py` | read_then_convert 最近一次 SkillResult，包含读取来源、转换格式和生成文件路径。 | JSON 对象 |
| `python_sandbox_result.json` | `b2_run_skill.py` | python_sandbox 最近一次 SkillResult，包含 stdout、stderr、returncode 和运行限制状态。 | JSON 对象 |
| `python_unit_test_sandbox_result.json` | `b2_run_skill.py` | python_unit_test_sandbox 最近一次 SkillResult，包含 passed/failed 统计和逐用例结果。 | JSON 对象 |
| `web_search_result.json` | `b2_run_skill.py` | web_search 最近一次 SkillResult，包含查询语句、搜索来源和排序结果列表。 | JSON 对象 |
| `skill_run_log.jsonl` | `b2_run_skill.py` | 基础和扩展 Skill 的累计运行日志。 | JSONL |
